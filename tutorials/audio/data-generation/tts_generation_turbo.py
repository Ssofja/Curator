...
class ChatterboxTurboTTSGeneration(ProcessingStage):
    """
    Processor that generates audio using Chatterbox-Turbo model for individual utterances.
    
    Chatterbox-Turbo is faster and more efficient than the original Chatterbox:
    - 350M parameters (vs 500M)
    - Lower compute and VRAM requirements
    - Native paralinguistic tags support ([laugh], [chuckle], etc.)
    - Optimized for low-latency voice agents
    
    This processor takes a manifest containing individual utterances with speaker
    and text information, generates audio for each utterance using Chatterbox-Turbo TTS
    ...
    def __init__(
        ...
        self.temp_dir = tempfile.mkdtemp(prefix="chatterbox_turbo_processed_audio_")
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.info("Chatterbox-Turbo model loaded successfully (350M params, optimized for low-latency)")
            
        except ImportError as e:
            logger.error("Chatterbox-Turbo not installed. Please install: pip install chatterbox-tts")
            raise
        except Exception as e:
            logger.error(f"Error loading Chatterbox-Turbo model: {e}")
            raise
    ...
    def _generate_audio_for_turn(self, text: str, speaker: str, reference_wav: str, conversation_id: str = "unknown") -> np.ndarray:
        """
        Generate audio for a single conversation turn using Chatterbox-Turbo.
        
        Chatterbox-Turbo supports paralinguistic tags natively:
        ...
        """
        try:
            logger.info(f"Generating audio for speaker '{speaker}': '{text[:50]}...'")
            
            with torch.inference_mode():
                # Chatterbox-Turbo simplified API
    ...
    def process_dataset_entry(self, data_entry: Dict) -> Dict:
        ...
        if 'utterance' in data_entry:
            ...
            raise ValueError("No text or utterance found in data entry")    
